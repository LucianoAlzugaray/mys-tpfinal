...
class Dia:
    def __init__(self, minutos_maximo, camionetas):
        ...
        self.tiempo_actual = Simulacion.TIEMPO_INICIO
    ...
    def correr(self):
        while not self.termino_dia():
            for evento in self.obtener_eventos_de_ahora():
                evento.notify()
            self.tiempo_actual += timedelta(minutes=1)
    ...
    def termino_dia(self):
        from Simulacion import Simulacion
        return self.tiempo_actual > Simulacion.TIEMPO_FIN
    ...
    def obtener_eventos_de_ahora(self):
        from Simulacion import Simulacion
        # TODO: actualizar self.tiempo_actual usando el reloj
        return list(filter(lambda x: x.hora == self.tiempo_actual, self.fel))
